...
export const initialPurchaseOrderReturnState: PurchaseOrderReturnState = {
  purchaseOrders: [],
  isLoading: false,
  loadList: false,
  isAddUpdate: false,
  purchaseOrderResourceParameter: {
    ...
    productId: '',
    productName: '',
    status: PurchaseOrderStatusEnum.Return,
    locationId: '',
    pageSize: 30,
...
export const PurchaseOrderReturnStore = signalStore(
  { providedIn: 'root' },
  withState(initialPurchaseOrderReturnState),
  withComputed(({ }) => ({
  })),
      ...
      addUpdatePurchaseOrder: rxMethod<PurchaseOrder>(
        pipe(
          ...
          switchMap((purchaseOrder: PurchaseOrder) => {
            return purchaseOrderService.updatePurchaseOrderReturn(purchaseOrder).pipe(
              tapResponse({
                next: () => {
                  toastrService.success(translationService.getValue('PURCHASE_ORDER_RETURN_ADDED'));
                  patchState(store,
                    {
                      isLoading: false,
                      loadList: true,
