...
describe('PurchaseOrderRequestAddEditComponent', () => {
  ...
  function makeExistingOrder(): PurchaseOrder {
    return {
      id: 'por-9',
      orderNumber: 'POR-9',
      supplierId: 's1',
  ...
  beforeEach(async () => {
    ...
    purchaseOrderService = jasmine.createSpyObj<PurchaseOrderService>('PurchaseOrderService', ['getNewPurchaseOrderNumber']);
    ...
    await TestBed.configureTestingModule({
      imports: [PurchaseOrderRequestAddEditComponent, TranslateModule.forRoot()],
      providers: [
        provideRouter([]),
        CurrencyPipe,
        ...
        { provide: PurchaseOrderRequestStore, useValue: storeStub },
        {
  ...
  function create(routeData: any = {}): void {
    ...
    purchaseOrderService.getNewPurchaseOrderNumber.and.returnValue(of({ orderNumber: 'POR-100' } as any));
  ...
  it('new request builds the form, fetches the next order number and defaults Pending', fakeAsync(() => {
    ...
    expect(component.purchaseOrderForm.get('orderNumber')?.value).toBe('POR-100');
  ...
  it('edit mode patches the request, disables location input and patches items', fakeAsync(() => {
    create({ purchaseorder: makeExistingOrder() });
    expect(component.isEdit).toBeTrue();
    expect(component.purchaseOrderForm.get('orderNumber')?.value).toBe('POR-9');
  ...
  it('onProductSelection adds a row priced from purchasePrice and clears the product control', fakeAsync(() => {
    ...
    // characterization: this component seeds the TYPO'D control 'taxPercenatage';
    // getAllTotal patches 'taxPercentage' which patchValue silently ignores
    expect(item.get('taxPercenatage')?.value).toBe(10);
    expect(item.get('taxPercentage')).toBeNull();
  ...
  it('getAllTotal computes totals with round-off and rounds item totals', fakeAsync(() => {
    create();
    component.onProductSelection(makeProduct());
  ...
  it('onTotalChange back-computes the unit price from the entered total', fakeAsync(() => {
    ...
    component.onProductSelection(makeProduct());
    const item = component.purchaseOrderItemsArray.at(0);
    // onTotalChange reads 'taxPercentage' which does not exist in this variant
    // (typo'd 'taxPercenatage') -> percentage is 0, so total/quantity directly
    item.patchValue({ quantity: 2, discountPercentage: 0, total: '160' });
  ...
  it('new request submit saves through the store flagged as a purchase order request', fakeAsync(() => {
    create();
    component.purchaseOrderForm.patchValue({ supplierId: 's1', locationId: 'l1' });
    component.onProductSelection(makeProduct());
    component.purchaseOrderItemsArray.at(0).patchValue({ quantity: 2, discountPercentage: 5 });
    ...
    expect(saved.isPurchaseOrderRequest).toBeTrue();
  ...
  it('store add/update completion navigates to the request list', fakeAsync(() => {
    ...
    fixture.detectChanges();
    flush();
    expect(router.navigate).toHaveBeenCalledWith(['/purchase-order-request/list']);
  }));
