...
export class PurchasePaymentReportDataSource
  implements DataSource<PurchaseOrderPayment> {
  ...
  loadData(purchaseOrderResource: PurchaseOrderResourceParameter) {
    ...
    this.sub$ = this.purchasePaymentReportService
      .getAllPurchaseOrderPaymentReport(purchaseOrderResource)
      .subscribe((resp: HttpResponse<PurchaseOrderPayment[]>) => {
        ...
        if (resp && resp.body) {
          ...
          this._purchaseOrderSubject$.next(expenses);
        }
      });
  }
}
