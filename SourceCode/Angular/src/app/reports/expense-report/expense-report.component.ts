...
export class ExpenseReportComponent extends BaseComponent implements OnInit {
  ...
  getBusinessLocations() {
    this.commonService.getLocationsForReport().subscribe((locationResponse) => {
      ...
      if (this.locations?.length > 0) {
        ...
        this.dataSource.loadData(this.expenseResource);
        this.searchForm.get('locationId')?.setValue(this.expenseResource.locationId);
      }
    });
  }
  ...
  getUsers() {
    this.sub$.sink = this.commonService.getAllUsers().subscribe((resp: User[]) => {
      this.users = resp;
    });
  }
    ...
    this.expenseService
      ...
      .subscribe((c: HttpResponse<Expense[]>) => {
        if (c.body) {
          ...
          const title = this.translationService.getValue('EXPENSE_REPORT');
          if (type === 'csv' || type === 'xlsx') {
            ...
            const locationName = this.locations.find(
              (x) => x.id == this.expenseResource.locationId
            )?.name;
            let y = 15;
            doc.text(
            ...
            if (this.expenseResource.fromDate) {
              dateFilter = `${this.translationService.getValue(
                'FROM'
              )}::${this.utcToLocalTime.transform(this.expenseResource.fromDate, 'shortDate')}`;
            }
            if (this.expenseResource.toDate) {
              dateFilter =
                dateFilter +
                `   ${this.translationService.getValue('TO')}::${this.utcToLocalTime.transform(
                  this.expenseResource.toDate,
                  'shortDate'
                )}`;
            }
            if (dateFilter) {
