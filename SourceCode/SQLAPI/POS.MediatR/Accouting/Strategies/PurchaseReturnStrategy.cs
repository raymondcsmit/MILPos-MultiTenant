    ...
    ITaxRepository _taxRepository) : IPurchaseReturnStrategy
{
    ...
    public async Task ProcessTransactionAsync(Transaction transaction)
    {
        ...
        // Purchase Return Entry: Dr. Accounts Payable, Cr. Inventory
        var mainEntry = await _accountingEntryFactory.CreateEntryAsync(
            ...
            $"Purchase Return - {transaction.Narration}",
        ...
        foreach (var kvp in taxTotal)
        {
            ...
            if (totalAmount > 0)
            {
                var gstEntry = await _accountingEntryFactory.CreateEntryAsync(
                    ...
                    gstAccountId,
                    totalAmount,
                    $"{accountName} on Purchase return - {transaction.Narration}",
                    transaction.ReferenceNumber,
                     transaction.FinancialYearId,
                    EntryType.Tax);
        ...
        // Discount Entry: CR. Accounts Payable, DR. Discount Received
