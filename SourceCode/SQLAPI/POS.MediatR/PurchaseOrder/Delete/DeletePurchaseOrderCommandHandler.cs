...
namespace POS.MediatR.Handlers
{
       ...
       : IRequestHandler<DeletePurchaseOrderCommand, ServiceResponse<bool>>
    {
        ...
        public async Task<ServiceResponse<bool>> Handle(DeletePurchaseOrderCommand request, CancellationToken cancellationToken)
        {
            try
            {
                var purchaseOrder = await _purchaseOrderRepository.All
                    .Include(c => c.PurchaseOrderItems)
                        .ThenInclude(c => c.PurchaseOrderItemTaxes).FirstOrDefaultAsync(c => c.Id == request.Id);
                ...
                {
                    return ServiceResponse<bool>.Return409("purchase Order can't delete becuase it's already Return.");
                }
                ...
                { 
                    var transactions = await _transactionRepository.All
                        .Where(c => c.ReferenceNumber == purchaseOrder.OrderNumber)
            ...
            {
                _logger.LogError(ex, "error while Deleting PurchaseOrder");
                return ServiceResponse<bool>.Return500("error while Deleting PurchaseOrder");
            }
