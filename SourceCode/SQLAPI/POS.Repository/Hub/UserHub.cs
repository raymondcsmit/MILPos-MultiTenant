...
namespace POS.Repository
{
    public class UserHub : Hub<IHubClient>
    {
        ...
        public async Task OnUserPermissionChange(Guid userId)
        {
            var userInfoReciever = _userInfoInMemory.GetUserInfoById(userId);
            await Clients.Client(userInfoReciever.ConnectionId).OnUserPermissionChange(userId);
        }
        ...
        public override async Task OnDisconnectedAsync(Exception exception)
        {
            ...
            if (userInfo == null)
                return;
            _userInfoInMemory.Remove(userInfo);
            await Clients.AllExcept(new List<string> { userInfo.ConnectionId }).UserLeft(userInfo.Id.ToString());
        }
    }
}
