...
| N-27 | 🟡 | **Supplier duplicate → 422, unlike every other CRUD module (409)**: `AddSupplierCommandHandler`/`UpdateSupplierCommandHandler` return `Return422` (Brand/Unit/ProductCategory use `Return409`). Supplier create/update also require the **non-nullable** `BillingAddressId`/`ShippingAddressId` FKs (payload omitting them → FK violation 500). And `UnitConversationController` has **no get-by-id route** — soft-delete visibility is pinned via the list, not a 404. All three pinned by `UnitConversationCrudTemplateTests`/`ProductCategoryCrudTemplateTests`/`SupplierCrudTemplateTests` (21 tests) | W2 CRUD |
| N-28 | 🟡 | **LocationController flattens 409 into HTTP 400**: `AddLocation` returns `BadRequest(response)` instead of the shared `ReturnFormattedResponse` — the handler's conflict surfaces as HTTP 400 with `statusCode:409` + `"Location Already Exist."` in the body (every other CRUD controller exposes the real status). Also `Locations.FBRKey` is NOT NULL — create payloads omitting it fail with `NOT NULL constraint failed: Locations.FBRKey`. Pinned by `LocationCrudTemplateTests` | W2 CRUD |
| N-29 | 🟠 | **Customer MobileNo uniqueness is an unguarded DB index**: `Customers(TenantId, MobileNo)` is a UNIQUE index, but `AddCustomerCommandHandler` only guards `CustomerName` (422) — a repeat MobileNo in the same tenant explodes into a generic 500 (`UNIQUE constraint failed: Customers.TenantId, Customers.MobileNo`). There is no 409/422 safety net. Pinned by `Should_Return500_When_CreatingSecondCustomerWithSameMobile` | W2 CRUD |
